...
const stabij=Array.from(breedCatalog).find(x=>x.name==='Friese Stabij (Stabijhoun)');assert.ok(stabij,'Friese Stabij missing');assert.ok(Array.from(stabij.aliases).includes('Stabijhoun'),'Stabijhoun alias missing');assert.ok(stabij.popular,'Friese Stabij should be marked popular');assert.ok(Array.from(breedCatalog).find(x=>x.name==='Labrador Retriever')?.popular,'Popular breed marker missing');
