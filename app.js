...
function setupReports(){
  ...
  el('reportForm').addEventListener('submit',e=>{e.preventDefault();if(!selectedReportType){toast('Kies eerst wat je hebt gespot');return}if(selectedReportType==='vegetation'&&!selectedVegetation){toast('Welke vegetatie heb je gezien?');return}if(selectedReportType==='lost'&&!selectedLostKind){toast('Kies vermist of gevonden');return}const center=map.getCenter(),p=profile(),sub=selectedReportType==='lost'?(selectedLostKind==='missing'?'Vermiste hond':'Gevonden hond'):selectedVegetation,text=el('reportText').value.trim()||defaultReportText(selectedReportType,sub),reports=allReports();reports.push({id:crypto.randomUUID?.()||String(Date.now()),type:selectedReportType,subtype:sub,text,lat:center.lat,lng:center.lng,time:'Zojuist',author:p?.name||'Anonieme hond',confirmed:0});saveJSON(STORAGE.reports,reports);drawReports();updateProfileUI();el('reportDialog').close();toast('🐾 Dankjewel! Je melding staat op de kaart.')})
}
...
function setupChat(){
  ...
  const openConversation=item=>{item={id:item?.id||'general',avatar:item?.avatar||'🐾',name:item?.name||'Buurtgesprek',text:item?.text||'Start een nieuw gesprek in de buurt.',time:item?.time||'Nu'};activeChat=item;el('chatList').classList.add('hidden');el('chatConversation').classList.remove('hidden');el('chatInput').placeholder=`Bericht aan ${item.name}…`;const history=[{text:item.text,time:item.time},...messages.filter(m=>m.chatId===item.id&&m.text).map(m=>({text:m.text,time:'Nu'}))];el('chatConversation').innerHTML=`<div class="conversation-head"><button type="button" class="conversation-back" aria-label="Terug naar gesprekken">‹</button><div class="chat-avatar">${item.avatar}</div><div><b>${escapeHTML(item.name)}</b><small>Whatsup dog · buurtgesprek</small></div><button type="button" class="text-action" id="clearConversation">Wis gesprek</button></div><div class="conversation-messages">${history.map((m,i)=>`<div class="message-bubble ${i===history.length-1&&history.length>1?'mine':''}">${escapeHTML(m.text)}<small>${m.time||'Nu'}</small></div>`).join('')}</div>`;el('chatConversation').querySelector('.conversation-back').onclick=()=>{activeChat=null;el('chatConversation').classList.add('hidden');el('chatList').classList.remove('hidden');el('chatInput').placeholder='Zeg iets hondigs…'};el('clearConversation').onclick=()=>{if(!confirm(`Dit gesprek met ${item.name} wissen?`))return;messages=messages.filter(m=>m.chatId!==item.id);saveJSON(STORAGE.chat,messages);activeChat=null;el('chatConversation').classList.add('hidden');el('chatList').classList.remove('hidden');el('chatInput').placeholder='Zeg iets hondigs…';render();toast('Gesprek gewist')};};
  ...
  el('clearChat')?.addEventListener('click',()=>{if(!messages.length){toast('Er zijn geen eigen berichten om te wissen');return}if(!confirm('Alle lokaal opgeslagen berichten wissen?'))return;messages=[];saveJSON(STORAGE.chat,messages);activeChat=null;el('chatConversation').classList.add('hidden');el('chatList').classList.remove('hidden');el('chatInput').placeholder='Zeg iets hondigs…';render();toast('Lokale berichten gewist')});
}
