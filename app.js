...
function deleteReport(r){if(!confirm('Deze melding wissen?'))return;saveJSON(STORAGE.reports,allReports().filter(x=>x.id!==r.id));drawReports();updateProfileUI();el('detailDialog')?.close();toast('Melding gewist')}
function deleteReport(r){if(!confirm('Deze melding van dit toestel wissen?'))return;saveJSON(STORAGE.reports,allReports().filter(x=>x.id!==r.id));drawReports();updateProfileUI();el('detailDialog')?.close();toast('Melding gewist')}
