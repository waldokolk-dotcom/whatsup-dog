...
-- Idempotent recovery for hosted projects that are behind repository migrations.
alter table public.profiles
  add column if not exists discoverable boolean not null default false,
  add column if not exists breed text check (breed is null or char_length(breed)<=80);

-- Reassert privacy-safe semantics even when an older hosted column has drifted.
update public.profiles set discoverable=false where discoverable is null;
alter table public.profiles alter column discoverable set default false;
alter table public.profiles alter column discoverable set not null;
...
begin
  ...
  if not exists(select 1 from public.profiles p where p.id=target and p.discoverable=true) then raise exception 'Profile is not discoverable' using errcode='42501'; end if;
