(()=>{
  ...
  function stopPick({clear=false}={}){if(map)map.off('click',handleMapClick);S.pickMode=null;S.toolbar?.remove();S.toolbar=null;document.querySelector('.map-crosshair')?.remove();if(clear){ensureDraftLayer()?.clearLayers();S.draftPoints=[]}}
  ...
  function makeToolbar(mode){
    ...
    if(mode==='point')bar.innerHTML='<b>📍 Verschuif de kaart onder de markering</b><button type="button" class="draw-center">✓ Kies deze plek</button><button type="button" class="draw-cancel">× Stop</button>';
    else bar.innerHTML='<b>🐾 Kies een gebied</b><button type="button" class="draw-center">◎ Gebruik kaartgebied</button><button type="button" class="draw-undo">↶ Wis</button><button type="button" class="draw-cancel">× Stop</button><button type="button" class="draw-done">✓ Klaar</button>';
    document.getElementById('view-map')?.appendChild(bar);S.toolbar=bar;bar.querySelector('.draw-cancel').onclick=cancelPick;
    if(mode==='point')bar.querySelector('.draw-center').onclick=()=>{const c=map.getCenter();S.point={lat:Number(c.lat.toFixed(6)),lng:Number(c.lng.toFixed(6))};stopPick({clear:false});updateLocationStatus();reopenReport()};
  ...
  function saveReport(e){
    ...
    const report={id:crypto.randomUUID?.()||String(Date.now()),type:selectedReportType,species,subtype,text:$('reportText').value.trim()||defaultReportText(selectedReportType,subtype),lat:loc.lat,lng:loc.lng,time:'Zojuist',createdAt:new Date().toISOString(),author:p?.name||'Anoniem',confirmed:0,geometryType:S.geometryMode==='area'?'polygon':'point',polygon:S.geometryMode==='area'?S.polygon:null,photoDataUrl:S.photo||null,photoName:S.photoName||null,aiSuggestion:S.ai||null,photoPrivacy:'canvas-reencoded-no-original-exif',_shareIntent:true};
  ...
  function openEnhancedDetail(r){const t=reportTypes.find(x=>x.id===r.type)||reportTypes[0],photo=r.photoDataUrl?`<img class="smart-photo-in-detail" src="${escapeHTML(r.photoDataUrl)}" alt="Foto bij melding">`:'',area=r.geometryType==='polygon'?`<div class="report-area-tag">✏️ Ingetekend gebied · ${r.polygon?.length||0} punten</div>`:'',ai=r.aiSuggestion?`<div class="ai-detail-note"><b>✨ Beeldsuggestie:</b> ${escapeHTML(r.aiSuggestion.label||'Onbekend')} ${Number.isFinite(r.aiSuggestion.confidence)?`(${Math.round(r.aiSuggestion.confidence*100)}%)`:''}<br>Geen vaststelling dat iets veilig of giftig is.</div>`:'';const canManage=!r._remote||(r.userId&&r.userId===window.WhatsupDogCommunity?.user?.id);const ownerActions=canManage?'<div class="detail-actions"><button type="button" class="outline-btn" id="resolveReport">✓ Melding opgelost</button><button type="button" class="outline-btn danger-action" id="deleteReport">⌫ Melding wissen</button></div>':'';$('detailContent').innerHTML=`<div class="detail-icon">${t.icon}</div><h2>${escapeHTML(r.subtype||t.label)}</h2><div class="detail-meta">${escapeHTML(r.time)} · gemeld door ${escapeHTML(r.author||'hondenbezitter')}</div>${area}${photo}${ai}<p class="detail-body">${escapeHTML(r.text)}</p>${ownerActions}`;$('detailDialog').showModal();if(canManage){$('resolveReport').onclick=()=>markReportResolved(r);$('deleteReport').onclick=()=>deleteReport(r)}}
